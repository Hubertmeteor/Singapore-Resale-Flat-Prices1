...
icon = Image.open("mongodb.png")
st.set_page_config(page_title= "Project1-Youtube Data Harvesting and Warehousing",
                   page_icon= icon,
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """# Youtube Data Harvesting and Warehousing*"""})
...
with st.sidebar:
    selected = option_menu(None, ["Home","Extract and Transform","View"], 
                           ...
                           styles={"nav-link": {"font-size": "30px", "text-align": "centre", "margin": "0px", 
                                   ...
                                   "nav-link-selected": {"background-color": "#c8b101"}})
...
mydb = psycopg2.connect(
    ...
    port="5432"        # Replace with the PostgreSQL server's port
   )
mycursor = mydb.cursor()

#  YOUTUBE API Connection
api_key='AIzaSyDwFEaeXlh9epVRoEv88Tr8-iWMHugA4zA'
...
# EXTRACT TAB
    with tab1:
        ...
        ch_id = st.text_input("Hint : Goto channel's home page > About > click share icon > copy channel_id").split(',')
...
# TRANSFORM TAB
    with tab2:     
        st.markdown("#   ")
        st.markdown("### Select a channel to begin Transformation to postgreSQL")
        ...
        if st.button("Submit"):
            try:
                ...
                st.success("Transformation to postgreSQL Successful!!!")
            except:
...
if selected == "View":
    ...
    st.write("## :orange[Select any question from the drop down]")
